...
| All Boards Statistics | Switches to a board table containing timestamp, trigger ID, trigger rate, lost trigger %, event-build %, and data rate. | **Partial/replaced.** The table shows the 8 ns hardware timestamp in seconds, trigger ID, received-event rate, estimated lost-trigger total/percentage derived from trigger-ID gaps, decoded T-OR rate, and decoded-payload rate. Event-build percentage is omitted because this DAQ does not build events. Rates describe decoded data rather than unobserved board/transport totals. |
...
| Run number spinbox | Selects numeric run 0–10000 and participates in jobs/file naming. | **Replaced.** SQLite atomically assigns each web run a monotonically increasing numeric ID; the version-one frontend supplies the internal actor identity `operator`. |
| Plot Type | Selects PHA LG/HG, ToA, ToT, channel trigger rate, MCS, waveform, 2-D trigger/charge, staircase, or hold-delay plots. | **Partial.** Typed live PHA LG/HG, ToA, and ToT datasets are rendered with uPlot, including channel overlays, zoom, cursor values, and linear/log Y scales. Other families remain missing. |
