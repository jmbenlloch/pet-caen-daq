...
- `task hardware:inspect` validates an already-ready four-link topology and
  reads DT5215 software/FPGA/PID identity plus board product, FPGA firmware,
  and acquisition-status registers without any hardware write, reset,
  enumeration, synchronization, API bind, or run storage mutation;
...
Run-control service errors now use stable bracketed diagnostic codes with consistent ConnectRPC status mapping: invalid identity/configuration uses `InvalidArgument`, existing directories use `AlreadyExists`, lifecycle/configuration failures use `FailedPrecondition`, overlapping operations use `Aborted`, and catalog/storage failures use `Internal`. Start requests no longer accept an operator-selected run ID: SQLite atomically allocates monotonically increasing numeric IDs above all indexed numeric runs. Stop remains idempotent for the same completed run, storage still rejects an unexpected existing directory, and concurrent start/stop requests are serialized with immediate rejection. The frontend supplies the fixed version-one actor identity `operator` without exposing an editable requester field.
