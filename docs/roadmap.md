...
Progress as of 2026-07-21: read-only discovery/topology validation, native protected-flash pedestal loading, core slow-control including register writes and synchronization/start/stop commands, the production configuration parser and FPGA/Citiroc/probe/HV/pedestal configuration planning/application paths, a complete persisted production semantic audit, incremental descriptor-table stream decoding, complete source-confirmed DT5202 event qualifier decoding, Run 54 format-3.4 compatibility decoding, register-sensitive simulator generation for all DT5202 event modes, explicit bounded stop-and-drain behavior, deterministic control/stream/framing/drain fault scripts, complete-batch raw capture plus lower-level transport journaling/replay, lightweight JSON/JSON Lines run storage, and a complete persisted one-pulse run are implemented. The Citiroc verification audit remains.
