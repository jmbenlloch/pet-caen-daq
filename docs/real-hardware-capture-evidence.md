...
16. **Run-start latency:** workspace captures `first_run_go.pcap` and `janus_config.pcap` show that the original native start path spent about 11.20 s repeating a hard four-board configuration and another 5.03 s in `SNT0`; the remaining reset/start/train sequence took about 72 ms. JANUS separated explicit configuration from run start, did not repeat link synchronization for an already synchronized session, and reached running in about 7.2 s. Native run control now reuses the last successfully applied configuration only when the parsed ordered assignments are unchanged, and skips `SNT0` only when every discovered board reported the `StatusTDLinkSynchronized` bit for the current connection. A changed assignment, reconnect, or missing synchronization evidence retains the conservative hard configuration or synchronization path.
17. **Per-session pedestal evidence:** `run_go_fastconfig.pcap` confirms that a cache-miss hard configuration reread the unchanged 272-byte protected-flash pedestal page on every board, contributing 1,128 writes and 1,088 reads. Pedestal calibration is immutable during ordinary reset/configure/start/stop operations, so the configurator now reads and validates each board once per hardware connection and reuses that evidence across later configuration changes. Reconnect creates a new configurator and therefore forces fresh reads.
18. **Consecutive-run command timing:** `fault.pcap` records a successful 15-second native run followed by a second start whose broadcast `RESET_PTRG (0x17)` returned DT5215 status 26 (`CNC_STATUS_TIMEOUT`). The client had submitted each command immediately after the concentrator's early acknowledgement even though the encoded delay scheduled execution 10 ms later. JANUS captures and FERSlib wait for the scheduled delay while retaining control-command serialization. Native FCMD/DCMD exchange now does the same, with context-cancelable waiting, so reset-time, reset-periodic, start, and stop commands cannot overlap their scheduled predecessors.
19. **Progress-aware drain:** `fault2.pcap` records four successful consecutive starts and stops after correcting command timing. On the fourth stop, the concentrator accepted `ACQ_STOP`, but forward-version service events on two chains did not expose ready status and bounded persistence delayed the final silence probe beyond the five-second drain deadline. Drain completion now always requires the capture-confirmed 100 ms stream-idle period, treats service-ready events as evidence rather than an early exit, and allows the final idle probe after time spent preserving successfully read batches. A stop failure that moves the coordinator to fault also clears stale active-run telemetry.
20. **Cancellation-safe stream framing:** `run-28/transport.journal` proves that the normal reader was canceled during a payload at connection offset 6,003,804. Drain then consumed the payload remainder `ff ff ff ff ff ff ff ff 4a 06 f4 40` as a new header and diagnosed the derived row count 4,256,774. The DT5215 client now retains partial header, descriptor-table, and payload bytes across cancellation and removes them only after assembling a complete validated batch, so the run-to-drain handoff resumes the interrupted frame. Transport journaling remains byte-exact across the termination/resumption boundary.
21. **Repeated counting channels:** `pcap/run-60/manifest.json` records a counting-mode coordinator fault after 34 persisted events because chain 2 node 0 repeated channel 34 in one payload. Raw capture was disabled, so the offending bytes are not available and the repetition itself remains diagnostic evidence rather than capture-verified wire evidence. The bundled FERSlib decoder is source-confirmed to accept repeated channel, T-OR, and Q-OR words with last-value-wins semantics. The native decoder now matches that behavior while retaining one canonical counter per channel.
22. **Transient FCMD status retry:** `pcap/run-61` completed a 15-second counting run, while the immediately following `pcap/run-62` failed before stream acquisition when broadcast `RESET_PTRG` returned DT5215 status 26 (`CNC_STATUS_TIMEOUT`). Run 62's raw capture and transport journal contain headers only, confirming that no stream batch was available to decode. The bundled FERSlib control path is source-confirmed to retry an FCMD up to ten times after any nonzero DT5215 status; the native client previously treated the first status as fatal. FCMD/DCMD now retain control serialization and their scheduled delay across the same bounded retry policy.
23. **Consecutive counting-run FCMD failure:** `pcap/fcmderror.pcap` (SHA-256 `f6f4a11717e8d531799c93a2cd35818ed556e046ffa9e14016d261db2d046384`) captures complete control and stream traffic across consecutive counting runs. One start succeeds immediately; a later start receives five status-26 replies to broadcast `RESET_TIME` before succeeding on attempt six; the final start receives status 26 for all ten broadcast `RESET_PTRG` attempts. Failed attempts are approximately 52 ms apart, proving that the replies are concentrator-side timeouts rather than client-side response misalignment. The current bundled FERSlib TDL path stages reset, start, and stop as broadcast `DCMD`, waits 10 ms, and executes each staged command through concentrator register `VR_IO_SYNC_SEND`. Native TDL run and test-pulse control now use that source-confirmed synchronized path. Ordinary per-board commands and the test pulse itself remain FCMD.
