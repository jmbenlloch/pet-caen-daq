...
The native loader reads protected page 4 with the AT45DB321 `0xD2` main-memory page-read opcode. Its relevant 272-byte layout is: `P` tag at byte 0, format 0 at byte 1, little-endian year plus month/day at bytes 2--5, four little-endian DC offsets at bytes 6--13, two spare bytes, 64 LG values at byte 16, and 64 HG values at byte 144. The loader validates every field and returns the DC offsets as evidence; it does not apply them. It exposes no flash program or erase operation, and the simulator explicitly rejects the `0x82` page-program opcode.
