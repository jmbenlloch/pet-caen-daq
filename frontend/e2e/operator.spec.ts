...
test('operator completes a simulated run and downloads its persisted artifact', async ({
  ...
  await expect(page.locator('.board-card').filter({ hasText: 'Node 0' })).toHaveCount(4)
  await expect(page.getByText('DT5202', { exact: true })).toHaveCount(4)
  ...
  await plots
    .getByRole('button', { name: 'Board 0, chain 0 node 0, channel 1', exact: true })
    .click()
...
test('operator monitors and safely switches high voltage while ready', async ({ page }) => {
  ...
  await expect(hvPanel.getByLabel('Board 0, chain 0 node 0 HV: Off')).toBeVisible()
  ...
  await expect(hvPanel.getByLabel('Board 0, chain 0 node 0 HV: Ramping')).toBeVisible()
  await expect(board0.getByText('HV on')).toBeVisible()
  await expect(hvPanel.getByLabel('Board 0, chain 0 node 0 HV: On')).toBeVisible()
