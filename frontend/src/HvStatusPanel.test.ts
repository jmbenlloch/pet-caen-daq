...
describe('HV status panel', () => {
  it('shows independent board LEDs with fault priority globally', () => {
    ...
    expect(wrapper.get('[aria-label="Board 0, chain 0 node 0 HV: Off"]').classes()).toContain('off')
    expect(wrapper.get('[aria-label="Board 1, chain 1 node 0 HV: On"]').classes()).toContain('on')
    expect(wrapper.get('[aria-label="Board 2, chain 2 node 0 HV: Ramping"]').classes()).toContain(
      'ramping',
    )
    expect(wrapper.get('[aria-label="Board 3, chain 3 node 0 HV: Fault"]').classes()).toContain(
      'fault',
    )
  ...
  it('keeps ramp-up visible after the hardware ramp bit clears', () => {
    ...
    expect(wrapper.get('[aria-label="Board 0, chain 0 node 0 HV: Ramping"]').classes()).toContain(
      'ramping',
    )
  })
