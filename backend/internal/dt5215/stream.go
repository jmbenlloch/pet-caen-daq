...
func (c *Client) ReadStreamBatch(ctx context.Context) ([]StreamEvent, error) {
	...
	if _, err := io.ReadFull(c.stream, header); err != nil {
		return nil, fmt.Errorf("read stream batch header: %w", err)
	}
	...
	if _, err := io.ReadFull(c.stream, table); err != nil {
		return nil, fmt.Errorf("read stream descriptor table: %w", err)
	}
	var extent uint64
	for i := 0; i < rows; i++ {
		p := table[i*32:]
		...
		if size*4 > MaxStreamEventBytes {
			return nil, fmt.Errorf("descriptor %d payload too large", i)
		}
	...
	if extent > MaxStreamBatchBytes {
		return nil, fmt.Errorf("stream batch payload extent %d is too large", extent)
	}
	...
	return DecodeStreamBatch(batch)
}
